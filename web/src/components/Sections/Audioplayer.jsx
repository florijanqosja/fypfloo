...
export default function MainSection() {
  ...
  const fetchAudioData = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_DOMAIN_DEV}audio/to_label`);
  ...
  const deleteAudio = async () => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_DOMAIN_DEV}audio`, {
        data: {
          Sp_ID: audioID,
        },
      });
  ...
  const handleSubmit = async () => {
    try {
      await axios.put(`${process.env.REACT_APP_API_DOMAIN_DEV}audio/label`, {
        Sp_ID: audioID,
        Sp_LABEL: labelValue,
      });
